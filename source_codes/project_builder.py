...
#pyinstaller --onefile --noconsole -nproject_builder --distpath=. project_builder.py
...
class Bilder():
    ...
    def preparation_for_launch(self):
        try:    
            os.chmod(f'{self.auto_opener_dir}\\exe_file\\source_codes\\image\\icon.ico',\
                     stat.S_IWRITE)
    ...
    def create_exe_file(self):
        run(['--onefile', '-nAuto-opener', '--clean',\
             '--noconsole', f'--distpath={self.auto_opener_dir}\\exe_file', 'main.py'])
    ...
    def moving_construction_elements(self):
        ...
        with os.scandir(f'{self.source_codes_dir}\\image') as img_dir,\
            os.scandir(f'{self.source_codes_dir}\\save') as save_dir:
    
            {copy(f'{self.source_codes_dir}\\image\\{img.name}',\
             self.dict_dir['img_dir_path'])\
             for img in img_dir}
