...
class Constructor_gui(Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.save_url = f'{os.getcwd()}\\save\\save_url.txt' 
        self.save_open = f'{os.getcwd()}\\save\\save_open.txt' 
        
        self.add = ImageTk.PhotoImage(file=f"{os.getcwd()}\\image\\add.png")
        self.clear = ImageTk.PhotoImage(file=f"{os.getcwd()}\\image\\clear.png")
        self.save = ImageTk.PhotoImage(file=f"{os.getcwd()}\\image\\save.png")
    ...
    def widgets_creating(self, commands):
        ...
        #---------------------------------------------------------------------
        widget = self.widgets_constructor(Button, "gray25", "white")
        widget.config(image = self.save, command=lambda: self.save_content())
        widget.place(x = 2, y = 3, height = 40, width = 40)
        #---------------------------------------------------------------------
        widget = self.widgets_constructor(Button, "gray25", "white")
        widget.config(text='ОТКРЫТЬ', command=lambda: commands.Open())
        widget.place(x = 44, y = 3, height = 40, width = 172)
        #--------------------------------------------------------------------- 
        widget = self.widgets_constructor(Button, "gray25", "white")
        widget.config(text = 'ЗАКРЫТЬ', command = lambda: commands.Close())
        widget.place(x = 218, y = 3, height = 40, width = 172)        
        #---------------------------------------------------------------------
        widget = self.widgets_constructor(Button, "gray25", "white")
        widget.config(image = self.add, command = lambda: commands.adding_elem())
    ...
    def save_content(self, exit=False):
        ...
        for num in range(len(self.text_field[0].get(0,END))):
            if '/' in self.text_field[0].get(num):
                number_buttons += 1
                content += f'{self.text_field[0].get(num)}\n' 

        with open(self.save_open,'w') as file_open, open(self.save_url,'w') as file_url:
            file_url.write(f'buttons_len: {number_buttons}\n{content}')   
            file_open.seek(0)
    ...
    def starter_content(self, widget, commands):
        with open(self.save_url,"r") as file_url, open(self.save_open,"r") as file_open,\
             os.scandir(self.shortcuts_path) as direct:
            try:
                number_buttons = int(file_url.readlines()[0][13:])
                file_url.seek(0)
                content = file_url.readlines()[1:]    
            except IndexError:
...
def run_gui(x,y):
    ...
    window = gui.windows_constructor('Автооткрыватор', [392,301,x,y], [False, False],\
                                     "wheat4")
